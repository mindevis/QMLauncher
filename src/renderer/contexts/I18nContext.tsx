...
export function I18nProvider({ children }: { children: ReactNode }) {
  // Initialize language from localStorage synchronously, then try Electron config
  ...
  // Load language from Electron config on mount (async)
  useEffect(() => {
    const loadLanguageFromConfig = async () => {
      if (window.electronAPI) {
        try {
          const config = await window.electronAPI.getLauncherConfig()
          if (config?.language && (config.language === 'en' || config.language === 'ru')) {
            setLanguageState(config.language)
            localStorage.setItem('qmlauncher_language', config.language)
          }
        } catch (error) {

...
func (inst *Instance) Rename(new string) error {
	// Sanitize new instance name for filesystem compatibility
	new = SanitizeInstanceName(new)
...
type InstanceConfig struct {
	...
	// QMServer configuration
	QMServerHost         string `toml:"qmserver_host,omitempty" json:"qmserver_host,omitempty"         comment:"QMServer host address"`
	QMServerPort         int    `toml:"qmserver_port,omitempty" json:"qmserver_port,omitempty"         comment:"QMServer port"`
	IsUsingQMServerCloud bool   `toml:"is_using_qmserver_cloud,omitempty" json:"is_using_qmserver_cloud,omitempty" comment:"Whether this instance uses QMServer"`
...
func CreateInstance(options InstanceOptions) (Instance, error) {
	...
	options.Name = SanitizeInstanceName(options.Name)
	...
	optsPath := filepath.Join(inst.Dir(), "options.txt")
	if _, err := os.Stat(optsPath); os.IsNotExist(err) {
		// Default in-game language: Russian (Minecraft options.txt)
		_ = os.WriteFile(optsPath, []byte("lang:ru_ru\n"), 0644)
	}

	if inst.Loader == LoaderFabric {
		modsDir := filepath.Join(inst.Dir(), "mods")
		savedJar, err := meta.DownloadFabricAPIIfNeeded(modsDir, inst.CachesDir(), inst.GameVersion)
		if err != nil {
			_ = os.RemoveAll(inst.Dir())
			return Instance{}, fmt.Errorf("install Fabric API (required for Fabric): %w", err)
		}
		if savedJar != "" {
			RecordRemoteInstall(inst.Dir(), "mods", savedJar, RemoteInstallMeta{
				Category:  "mods",
				Source:    "modrinth",
...
// launcherInstanceLogTimestampSuffix matches the time.Format("2006-01-02_15-04-05") used in log filenames.
var launcherInstanceLogTimestampSuffix = regexp.MustCompile(`_\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}$`)

// instanceNameFromLauncherLog extracts the instance segment from qmlauncher_<name>_<timestamp>.log (GUI/TUI launch logs).
func instanceNameFromLauncherLog(filename string) (name string, ok bool) {
	const pref = "qmlauncher_"
	const suff = ".log"
	if !strings.HasPrefix(filename, pref) || !strings.HasSuffix(filename, suff) {
		return "", false
	}
	body := strings.TrimSuffix(strings.TrimPrefix(filename, pref), suff)
	idx := launcherInstanceLogTimestampSuffix.FindStringIndex(body)
	if idx == nil {
		return "", false
	}
	return body[:idx[0]], true
}

// removeInstanceLogs deletes ~/.qmlauncher/logs/qmlauncher_<instanceName>_<timestamp>.log for this instance.
func removeInstanceLogs(instanceName string) {
	if instanceName == "" {
		return
	}
	logsDir := filepath.Join(env.RootDir, "logs")
	...
	for _, e := range entries {
		...
		n := e.Name()
		logInst, ok := instanceNameFromLauncherLog(n)
		if !ok || logInst != instanceName {
			continue
		}
		_ = os.Remove(filepath.Join(logsDir, n))
	}
}

// RemoveInstance removes the instance with the specified name.
// Deletes the entire instances/<name>/ tree (including the UUID subfolder and the parent folder),
// not only the inner game directory — so no empty instance shell is left on disk.
func RemoveInstance(name string) error {
	inst, err := FetchInstance(name)
	if err != nil {
		return err
	}
	instanceRoot := filepath.Join(env.InstancesDir, inst.Name)
	if err := os.RemoveAll(instanceRoot); err != nil {
		return fmt.Errorf("remove instance directory: %w", err)
	}
	removeInstanceLogs(inst.Name)
	return nil
}
...
// RemovePlayerDirsForAccount removes players/<accountUUID> from all instances.
// Symlinks inside the player dir are removed (not followed) — target files stay intact.
func RemovePlayerDirsForAccount(accountUUID string) {
	if accountUUID == "" {
		return
	}
	...
	for _, inst := range insts {
		playerDir := filepath.Join(inst.Dir(), "players", accountUUID)
		if err := os.RemoveAll(playerDir); err != nil {
			continue
		}
	}
}

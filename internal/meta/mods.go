...
// CurseForgeMinecraftSearchURL is a fallback URL to search CurseForge when no direct project URL is known.
func CurseForgeMinecraftSearchURL(query string) string {
	q := strings.TrimSpace(query)
	if q == "" {
		return "https://www.curseforge.com/minecraft/mc-mods/"
	}
	return "https://www.curseforge.com/minecraft/search?search=" + url.QueryEscape(q)
}

// SearchModrinthProjectByType searches Modrinth with a project_type facet (e.g. resourcepack, shader, datapack) and returns the best matching project slug.
func SearchModrinthProjectByType(name string, projectType string, cachesDir string) (string, error) {
	name = strings.TrimSpace(name)
	if name == "" {
	...
	if projectType == "" {
		return "", fmt.Errorf("empty project type")
	}

	searchQuery := simplifyResourcePackNameForSearch(name)
	if searchQuery == "" {
		searchQuery = name
	}

	u, err := url.Parse("https://api.modrinth.com/v2/search")
	if err != nil {
		return "", err
	}
	v := u.Query()
	v.Set("query", searchQuery)
	v.Set("limit", "12")
	v.Set("facets", fmt.Sprintf(`[["project_type:%s"]]`, projectType))
	u.RawQuery = v.Encode()

	safeKey := strings.ReplaceAll(searchQuery, " ", "_")
	for _, ch := range []string{"/", "\\", ":", "<", ">", "|", "?"} {
		safeKey = strings.ReplaceAll(safeKey, ch, "_")
	}
	cachePath := filepath.Join(cachesDir, "modrinth", fmt.Sprintf("facet_%s_%s.json", projectType, safeKey))
	apiCache := network.Cache[ModrinthSearchResult]{
		Path:        cachePath,
		URL:         u.String(),
		AlwaysFetch: false,
	...
	searchTerms := strings.Fields(strings.ToLower(searchQuery))
	minMatches := len(searchTerms) * 3 / 5
	if minMatches < 1 && len(searchTerms) > 0 {
		minMatches = 1
	}

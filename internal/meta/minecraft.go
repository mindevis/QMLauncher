...
// ExtractGameVersionFromLoader extracts Minecraft game version from Forge/NeoForge loader version.
// E.g. "1.20.1-47.2.0" -> "1.20.1", "1.21.1-21.1.45" -> "1.21.1".
// Returns empty string if the format doesn't match.
func ExtractGameVersionFromLoader(loaderVersion string) string {
	// Pattern: digit.digit[.digit] followed by dash and loader build
	re := regexp.MustCompile(`^(\d+\.\d+(?:\.\d+)?)-`)
	if m := re.FindStringSubmatch(loaderVersion); len(m) >= 2 {
		return m[1]
	}
	return ""
}

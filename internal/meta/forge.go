...
func FetchNeoforgeVersion(gameVersion string) (string, error) {
	...
	resp, err := network.HTTPClientMetadata.Get(url)
	if err != nil {
		return "", err
	}
...
func FetchForgePromotions() (*orderedmap.OrderedMap, error) {
	resp, err := network.HTTPClientMetadata.Get("https://files.minecraftforge.net/net/minecraftforge/forge/promotions_slim.json")
	if err != nil {
		return nil, err
	}
	defer resp.Body.Close()
